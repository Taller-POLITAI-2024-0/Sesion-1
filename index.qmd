...
date: "2024-2-18"
date-modified: ''
author1: "**[Joel Ccallocunto](https://github.com/luccemhu)** \n a20196510@pucp.edu.pe & **[Carlos Crespín](https://github.com/krlos20203563)** \n  "
author2:  "auth"
theme:
  dark: darkly
  light: flatly
...
```{=html}
<!--
INPUT YAML: Ejemplo renderizar o knittear:
...
::: callout-note
Presesión: <https://taller-politai-2024-0.github.io/Sesion-0.1/>
:::
...
Pueden revisar los foros de la comunidad de programadores como [StackOverflow](https://stackoverflow.com/) en la que se realizan y absuelven dudas frecuentes o específicas.
...
```{r}
df <- data.frame(
  persona = c("Joel", "Santiago", "María Pía", "Ed"),
  nota = c(15, 17, 20, 16),
  aprueba = c(F, T, T, T))
df
```
...
#### Pero también encontraremos y, a veces, crearemos:
...
cars %>% table() %>% prop.table() * 100
```

# Importar base de datos en R... 

1. Desde archivos locales con la librería rio:

```{r}
library(rio) # Importar data, versatil
data_rio <- import("usa.sav")

# Usemos el pipe operator
# Lo exportamos en formato xlsx (para excel)

pacman::p_load(writexl) # Para exportar en excel

data_rio %>% write_xlsx("usa.xlsx")
```
...
```{r}
library(readxl)
link1 = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTWCF4WSPdI77-xZKDdufjlr6szhPe372FYleEPcJfPkj0JiRXFNyt9tS2KzcWqlA/pub?gid=1281997847&single=true&output=csv"
google_usa = read.csv(link1)
str(google_usa)
...
```{r}
# Estos datos nos dan información sobre ex convictos y el tiempo que demoran 
# en volver a la cárcel [Allison 2010]:
...
-   Mendoza, B. (2018). R para principiantes. Libro electrónico. <https://bookdown.org/jboscomendoza/r-principiantes4/>
...
Addison-Wesley Pub. Co. <https://archive.org/details/mackenzie-coded-char-sets>

-   Sotelo, S., & Van Oordt M., P. (2023). Laboratorio de Inteligencia Artificial y Métodos Computacionales en Ciencias Sociales - QLAB. <https://qlab.pucp.edu.pe/>
...
-  Alcántara, K.,  & Crispín, L. (2023). <https://karinaalc.github.io/Estadistica-2/pd1.html>

-   Tiscornia P. (2023). Introducción al procesamiento de datos con R para Ciencias Sociales. Aplicación práctica en la Encuesta Permanente de Hogares (EPH-INDEC). <https://intro-r-eph.netlify.app/bio.html>
